...
export class FindAllTvProgramsUseCase
	implements UseCase<unknown, GetterTVPrograms[]>
{
	async config(params) {
		const map = new Map()
		params.playlistId && map.set('playlistId', params.playlistId)
		params.idProduct && map.set('idProduct', params.idProduct)
		params.isNecessarySubCategory &&
			map.set('subCategory', { [Op.in]: params.subCategoryNumber })
		return map
	}
	...
	async execute(
		...
		const mapConfig = await this.config({
			playlistId,
			idProduct,
