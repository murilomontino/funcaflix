...
const CulturalProfilesNameScreen = ({ profiles, name }: Props) => {
	...
	const [data, setData] = useState<IGetterCulturalProfile[]>(
		profiles.slice(0, 10)
	)

	const deferredData = useDeferredValue(data)

	const [dataTotalSearch, setDataTotalSearch] = useState<
	...
	const hasMoreSearch =
		deferredDataSearch.length < dataTotalSearch.length && isSearch
	...
	const fetchMoreData = () => {
		setTimeout(() => {
			setData((state) => [
				...state,
				...profiles.slice(state.length, state.length + 10),
			])
		}, 200)
	}
	...
	return (
		<React.Fragment>
			...
			<div className="page-content mt-2 w-90 p-5">
				<ReactInfiniteScroll
					dataLength={
						isSearch ? deferredDataSearch.length : deferredData.length
					}
					next={fetchMoreData}
					hasMore={hasMore}
				...
				>
					<div className="d-flex align-items-center overflow-hidden flex-wrap">
						<For items={isSearch ? deferredDataSearch : deferredData}>
							{(item) => {
								return (
