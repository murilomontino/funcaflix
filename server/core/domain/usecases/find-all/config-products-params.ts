...
async function categoriesInArray(
	...
	const categoryArrayInt = categoryArrayString.map((category) =>
		parseInt(category, 10)
	)

	categoryDefault &&
		categoryArrayInt.push(parseInt(this.categoryDefault.toString(), 10))
...
export { config, configWhere, categoriesInArray }
