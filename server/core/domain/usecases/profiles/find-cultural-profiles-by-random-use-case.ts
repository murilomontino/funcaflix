	...
	implements UseCase<FindCulturalProfilesProps, IGetterCulturalProfile[]>
{
	...
	async execute({
		length,
	}: FindCulturalProfilesProps): PromiseEither<
		IGetterCulturalProfile[],
		Error
	> {
		const profilesRandomOrErr = await this.culturalProfileRepository.findRandom(
			length
		)

		if (profilesRandomOrErr.isRight())
			return right(
				new Error(
					'Erro no banco de dados: Não foi possível buscar aleatoriamente'
				)
			)
