...
export class FindAllPlaylistAndItemsUseCase
	implements UseCase<unknown, Response[]>
{
	constructor(
		private readonly getterTVPrograms: UseCase<unknown, GetterTVPrograms[]>,
		private readonly getterPlaylist: UseCase<
			unknown,
			GetterPlaylistTVPrograms[]
		>
	) {}
	...
	async execute({
		category,
		subCategory,
	}: Props): PromiseEither<Response[], Error> {
		const playlists = await this.getterPlaylist.execute({ category })

		if (playlists.isRight())
			return right(new Error('Erro em Gerar as Playlists'))
