	...
	implements UseCase<unknown, IGetterCulturalProfile[]>
{
	...
	async execute(
		...
		const profileSearch = await this.culturalProfileRepository.findSearch(
			search
		)

		if (profileSearch.isRight())
			return right(
				new Error('Erro no banco de dados: Não foi possível buscar os nomes')
			)
