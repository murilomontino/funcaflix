...
export class SetterInfoProduct implements IProduct {
  ...
  static async create(params: IProduct): PromiseEither<SetterInfoProduct, Error> {
    const entityOrErr: Either<SetterInfoProduct, Error> = await SetterInfoProduct.schema
      .validate(params, {
        abortEarly: false,
      })
      .then((paramsValidate) => {
