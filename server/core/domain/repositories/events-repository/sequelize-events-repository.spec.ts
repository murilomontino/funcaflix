...
import {
	SequelizeEventsRepository,
	generateEvent,
} from './sequelize-events-repository'
...
describe('Test Events Repository', () => {
	...
	it('should be throw err find all events (Mock Database) (Unitary)', async () => {
		...
		database.query = vi
			.fn()
			.mockRejectedValue(new Error('Error Query Database'))
	...
	it('should be throw error fn generate event (Unitary)', async () => {
		...
		expect(generateEvent(undefined as any)).rejects.toThrowError(
			MissingParamError
		)
	})
})
